...
document.addEventListener('DOMContentLoaded', () => {
        ...
        signalButton.addEventListener('click', () => {
            ...
            if (isNewGame) {
                ...
                buttons.forEach(button => {
                    ...
                    if (star) {
                        star.classList.add('hidden'); // Скрываем звезду при новой игре
                        star.classList.remove('star-animated'); // Удаляем класс анимации для следующей игры
                    }
                });
    ...
    function placeSafeCells(count) {
        ...
        buttons.forEach(button => {
            ...
            const star = button.querySelector('.star');
            if (star) {
                star.classList.add('hidden'); // Скрываем звезду перед новым размещением
                star.classList.remove('star-animated'); // Удаляем класс анимации для следующей игры
            }
        });
        ...
        safeCellsToShow.forEach((index, i) => {
            setTimeout(() => {
                ...
                const star = buttons[index].querySelector('.star');
                if (star) {
                    setTimeout(() => {
                        star.classList.add('star-animated'); // Добавляем класс после завершения анимации
                    }, 700); // 700ms = 0.3s (shrinkToStar) + 0.4s (growStar)
                }
